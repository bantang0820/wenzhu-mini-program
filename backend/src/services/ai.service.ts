...
1. 字数限制：全文必须严格控制在 200 字以内，越精炼越有力量。
2. 极简段落：每段最多只能有 1 到 3 句话！绝不能超过 3 句！
3. 呼吸感排版：只要意思稍有停顿，必须立刻换行（使用回车键）。多用短句，创造视觉上的留白和轻盈感。
4. 段落数量：必须且只能输出 4 个自然段；段落之间只允许空 1 行；不得输出小标题、编号、项目符号。
5. 视角约束：禁止使用“你/你们/家长/父母/我们”作为主语指令，只能用“我”。
...
1. 日记内容要优先吸收“用户五次复述”中的词汇和语义，并且必须原样复用其中至少 1 个连续短语（不少于 4 个连续汉字）。
2. 允许轻量提及“时间流动感”，但不要机械复述时间点。
3. 不要空泛鸡汤，不要模板化套话。
